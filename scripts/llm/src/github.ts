...
export class Repo {
    ...
    readme = () => this.octokit.rest.repos.getReadme({ owner: this.owner, repo: this.name }).then(({data}) => atob(data.content)).catch(console.error)

    details = () => this.octokit.rest.repos.get({ owner: this.owner, repo: this.name }).then(({data}) => data).catch(console.error)

    files = (recursive: boolean) =>
    this.octokit.rest.git.getTree({ owner: this.owner, repo: this.name, tree_sha: "main", recursive: recursive ? "1" : "0"})
...
export class RepoFileContentTool extends Tool {
    ...
    readonly description: string = "a github repository file loader. useful for when you need to read the content of a specific file. input should be the path of the file within the repository.";
