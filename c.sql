...
FROM
    (
                    ...
                    FROM
                        (
                            SELECT
                                ...
                                "_root.ar.root.contributors"."contributors" AS "contributors",
        ...
        FROM
            ...
            LEFT OUTER JOIN LATERAL (
                SELECT
                    COALESCE(JSON_AGG("contributors"), '[]') AS "contributors"
                FROM
                    (
                        SELECT
                            ROW_TO_JSON(
                                (
                                    ...
                                    FROM
                                        (
                                            SELECT
                                                "_root.ar.root.contributors.base"."github_user_id" AS "githubUserId"
                                        ) AS "_e"
                                )
                            ) AS "contributors"
                        FROM
                            (
                                SELECT
                                    *
                                FROM
                                    "public"."projects_contributors"
                                WHERE
                                    (("_root.base"."id") = ("project_id"))
                            ) AS "_root.ar.root.contributors.base"
                    ) AS "_root.ar.root.contributors"
            ) AS "_root.ar.root.contributors" ON ('true')
    ) AS "_root"